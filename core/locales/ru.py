RU = {
    ...
    "start.greeting_connected": "С возвращением! Бот подключён и работает.",
    "connection.connected_notice": "✅ Бот подключён и уже работает в вашем аккаунте.",
    "connection.disconnected_notice": "🔌 Бот отключён от вашего бизнес-аккаунта.",
    "connection.open_app_button": "Открыть",
    ...
    "emoji_status.request_prompt": "Чтобы бот мог обновлять ваш эмодзи-статус, откройте мини-приложение и разрешите доступ на странице «Аккаунт»:",
    ...
    "cmddesc.storysplit": "Просто разрезает фото на части — без публикации в Истории",
    "cmddesc.storyautopost": "Переключает публикацию по одному тайлу в день вместо сразу всех",
    "cmddesc.spam": "Отправляет несколько одинаковых сообщений подряд (лимит без зеркала — 100)",
    "cmddesc.stt": "Распознаёт речь из ГС или видео-кружка",
    "cmddesc.sw": "Переключает раскладку в сообщении",
    "spam.usage": "Формат: /spam [кол-во] [текст]\nЛимит без зеркала — {limit}.",
    "spam.already_running": "Рассылка уже идёт, дождитесь её завершения.",
    "spam.clamped": "Запрошено {requested}, но лимит без зеркала — {limit}. Отправляю {count}.",
