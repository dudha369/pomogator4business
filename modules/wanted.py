...
def render_wanted(image_bytes, name, charge):
    ...
    draw.rectangle([15, 15, _CANVAS_SIZE[0] - 15, _CANVAS_SIZE[1] - 15], outline="#3a2a1a", width=6)

    title_font = load_font(90, serif=True)
    title = "WANTED"
    title_width = draw.textlength(title, font=title_font)
    draw.text(((_CANVAS_SIZE[0] - title_width) / 2, 40), title, font=title_font, fill="#3a2a1a")

    sub_font = load_font(30, serif=True)
    sub = "DEAD OR ALIVE"
    sub_width = draw.textlength(sub, font=sub_font)
    draw.text(((_CANVAS_SIZE[0] - sub_width) / 2, 145), sub, font=sub_font, fill="#3a2a1a")

    photo_x = (_CANVAS_SIZE[0] - _PHOTO_SIZE) // 2
    photo_y = 210
    canvas.paste(photo, (photo_x, photo_y))
    draw.rectangle(
        [photo_x - 5, photo_y - 5, photo_x + _PHOTO_SIZE + 5, photo_y + _PHOTO_SIZE + 5],
        outline="#3a2a1a",
        width=5,
    )
    ...
    draw.text(((_CANVAS_SIZE[0] - name_width) / 2, name_y), name, font=name_font, fill="#3a2a1a")
    ...
    for line in charge_lines:
        ...
        draw.text(((_CANVAS_SIZE[0] - line_width) / 2, y), line, font=charge_font, fill="#3a2a1a")
        y += 34
    ...
    draw.text(((_CANVAS_SIZE[0] - reward_width) / 2, y + 30), reward, font=reward_font, fill="#3a2a1a")

    seal_center = (_CANVAS_SIZE[0] - 140, _CANVAS_SIZE[1] - 160)
    draw.ellipse(
        [seal_center[0] - 70, seal_center[1] - 70, seal_center[0] + 70, seal_center[1] + 70],
        outline="#8a1a1a",
        width=5,
    )
...
@command(name="wanted", aliases=["розыск"], module="wanted", description="Создаёт постер в розыск")
